...
if (streams.data.length === 0)
  throw new Error(
    "No stream links to stream from, please add a stream in config.mjs or set ilovemusic to true"
  );

const components = () => [
  ...streams.data
    .reduce(
      (options: Discord.StringSelectMenuOptionBuilder[][], stream, index) => {
...
client.on(Discord.Events.InteractionCreate, async (i) => {
  if (
    !config.roles.some((r) =>
      (i.member as Discord.GuildMember).roles.cache.has(r)
    )
  )
    return;
  player.stop(true);
  ...
  player.play(
    Voice.createAudioResource(
      i.isButton()
        ? (streams as any)[i.customId].url
        : streams.jump(Number((i as any).values[0]))!.url
    )
  );
